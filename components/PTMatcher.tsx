...
function matchPTs(a) {
  // Map new 5-goal framework to specialism clusters
  const gMap = {
  ...
  return PT_ROSTER.filter(p => p.active && p.capacity > 0).map(p => {
    let s = 0;
    ts.forEach(x => { if (p.specialisms.includes(x)) s += 3; });
...
export default function PTMatcher({ mode = "member", sessionEmail = "" }) {
  ...
  async function submitForm(data){
    ...
    if (!res.ok) return;
    const out = await res.json();
    const matches = out.matches || matchPTs(data);
  ...
  const CSS = `
    ...
    * { box-sizing:border-box; margin:0; padding:0; }
    ...
    .h1  { font-family:'Horizon','Courier Prime',monospace; font-weight:400; font-size:clamp(38px,9vw,80px); line-height:0.92; letter-spacing:0.02em; text-transform:uppercase; }
