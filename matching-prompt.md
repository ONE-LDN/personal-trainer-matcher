...
5. **Injury flags matter.** If a client mentions an injury or medical condition, prioritise PTs with rehab, injury prevention or physiotherapy expertise where relevant. When an injury flag is present, **Craig and Mara are the top two candidates** — both rank above Adrian and Lucas in injury cases. Decide which of the two comes first based on the client's profile:
   - **Lean toward Craig** when the case is clinical, pain-led, or involves complex medical conditions — Craig is a fully qualified Master's-level osteopath with clinical S&C, and is the more senior practitioner on the roster for injury work. Remember Craig's exclusion (he does not take weight-loss or low-need clients).
   - **Lean toward Mara** when the case is rehab-to-performance, sport-return, or where the client is rebuilding after an injury and wants to keep training. Mara's physiotherapy background (MSc completion September 2026) makes her the strongest junior option for this kind of work.
   - When the case could reasonably go either way, treat them as equal top candidates.
